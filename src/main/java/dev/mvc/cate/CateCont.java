...
//http://localhost:9090/resort/cate/list_join.do
 ...
 public ModelAndView list_join() {
   ...
   * 
   * // Categrp: name, Cate: name 결합 목록 ArrayList<String> name_title_list = new
   * ArrayList<String>();
   * 
   * // 스트링버퍼의 경우 스트링에 대한 처리속도가 매우 빠르기 때문에 사용 StringBuffer url = new
   * StringBuffer(); // 카테고리 제목 링크 조합
   * 
   * // 카테고리 그룹 갯수 만큼 순환 for (int index = 0; index < categrp_list.size(); index++)
   * { CategrpVO categrpVO = categrp_list.get(index); // 하나의 카테고리 그룹 추출
   * 
   * name_title_list.add("<LI class='categrp_name'>"+ categrpVO.getName() +
   * "</LI>");
   * 
   * // 카테고리 Join 목록, 특정 카테고리 그룹에 해당하는 카테고리를 가져온다 List<Categrp_Cate_join>
   * cate_list = cateProc.list_join_by_categrpno(categrpVO.getCategrpno());
   * 
   * // 카테고리 갯수만큼 순환 for (int j=0; j < cate_list.size(); j++) { Categrp_Cate_join
   * categrp_Cate_join = cate_list.get(j);
   * 
   * String name = categrp_Cate_join.getName(); // 카테고리 이름 int cnt =
   ...
   * ")</span>"); url.append("</LI>"); name_title_list.add(url.toString()); // 출력
   * 목록에 하나의 cate 추가
   * 
   * url.delete(0, url.toString().length()); // 새로운 카테고리 목록을 구성하기위해 StringBuffer
   * 문자열 삭제
   * 
   * } }
   * 
