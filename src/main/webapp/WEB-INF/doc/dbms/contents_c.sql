...
INSERT INTO contents(contentsno, memberno, cateno, title, content, web, ip, passwd, word, rdate,
                              file1, thumb1, size1)
VALUES(contents_seq.nextval, 1, 5, '신규확진 2명', '전원 입국검역서 확인', 'http://www.daum.net',
...
SELECT contentsno, memberno, cateno, title, content, grpno, indent, ansnum, web, ip, passwd, word, rdate, file1, thumb1, size1
FROM contents 
...
2) 전체목록
SELECT contentsno, memberno, cateno, title, content, web, ip, passwd, word, rdate, file1, thumb1, size1, map
FROM contents 
...
DELETE FROM contents CASCADE CONSTRAINTS;
...
UPDATe contents
SET file1='file1', thumb1='thunb1', size1='size1'
...
답변 관련 종료
----------------------------------------------------------------------------------------------
SELECT c.contentsno, c.memberno, c.cateno, c.title, m.id
FROM contents c, member m
WHERE c.memberno = m.memberno;

SELECT contentsno, memberno, cateno, title, content, recom, cnt, replycnt, rdate, word, ip,
          file1, thumb1, size1,
          grpno, indent, ansnum, r, mid
FROM (
           SELECT contentsno, memberno, cateno, title, content, recom, cnt, replycnt, rdate, word, ip,
                     file1, thumb1, size1,
           ...
           FROM (
                     SELECT c.contentsno, c.memberno, c.cateno, c.title, c.content, c.recom, c.cnt, c.replycnt, c.rdate, c.word, c.ip,
                               ...
                               m.id as mid
                     FROM contents c, member m
                     WHERE cateno=29 AND c.memberno = m.memberno AND word LIKE '%봄%'
                     ORDER BY grpno DESC, ansnum ASC
           )          
)
WHERE r >= 1 AND r <= 10; 
