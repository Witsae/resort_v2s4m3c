    ...
    <UL>
      <c:choose>
        ...
        <c:otherwise> <!-- 패스워드 불일치 -->
          <LI class='li_none'>
            <span class='span_fail'>패스워드가 일치하지 않습니다. 다시 시도해주세요. cnt : ${cnt } passwd_cnt : ${passwd_cnt }</span>
          </LI>
        </c:otherwise>
      </c:choose>
      ...
      <c:choose>
        <c:when test="${cnt == 1 && passwd_cnt == 1}">
          <LI class='li_none'>
            <button type='button' 
                        onclick="location.href='./read.do?cateno=${param.cateno}&contentsno=${param.contentsno}&nowPage=${param.nowPage}'"
