...
function delete_form(attachfileno) {
  $('#btn_delete').attr('data-content', attachfileno); // 삭제할 파일번호 임시 저장
  
  msg = '파일을 삭제하시겠습니까? <br> 파일을 삭제하면 복구 할 수 없습니다. <br>';
  msg += '삭제 진행 시 삭제진행 버튼을 클릭하세요.';
...
function delete_proc() {
  var attachfileno = $('#btn_delete').attr('data-content');   // 삭제할 파일번호 호출
  var frm = $('#frm');
  $('#attachfileno', frm).val(attachfileno);// frm폼에서 태그를 검색
  frm.submit();
}
</script>
...
<form name='frm' id='frm' method='post' action='./delete.do'>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="button" id="btn_delete" data-focus="" data-content="" class="btn btn-default" data-dismiss="modal">삭제진행</button>
          <button type="button" id="btn_close" data-focus="" class="btn btn-default" data-dismiss="modal">취소</button>
        </div>
      </div>
    </div>
  </div> <!-- Modal 알림창 종료 -->
  ...
  <ASIDE class='aside_left'>
    <A href='./create.do?contentsno=${contentsno }&cateno=${cateno }'>첨부 파일 등록</A>
  </ASIDE>
